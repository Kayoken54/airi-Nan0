...
Nan0 activation is card-scoped through `extensions.airi.modules.cognition`. The official editor surface is `packages/stage-pages/src/pages/settings/airi-card/components/tabs/CardCreationTabCognition.vue`; `packages/stage-ui/src/stores/nan0-config.ts` owns the host discriminator (`local_nan0`) and provider/model routing contract. The generic `none` processor may use AIRI's host-owned first-hop pre-pass in `packages/stage-ui/src/stores/chat.ts`, while `local_nan0` is deliberately handled only by `packages/stage-ui/src/stores/nan0.ts` through the existing owner bridge. Disabled or non-Nan0 cards do not prepare thoughts, notify Nan0 input presence, or run Nan0 autonomous evaluation. The selected first-hop provider/model is resolved through AIRI's provider store; the kernel never owns provider credentials or transport.
...
Nan0's trusted time boundary is owned by `packages/nan0-runtime/src/temporal/Nan0Clock.ts` and `packages/nan0-runtime/src/temporal/Nan0Temporal.ts`. UTC wall time is used for restart-safe absence, sleep, deadlines, and due-condition eligibility; monotonic process time is used only for active computation duration. The persisted temporal record retains boot/shutdown/resume markers, timezone and offset, Kyo/Nan0 activity markers, clock confidence, discontinuity provenance, and bounded due-condition references. Timeline sequence remains authoritative when wall time moves backward or timezone/DST changes. Authoritative boot performs one bounded eligibility pass without creating an observation, thought, decision, provider request, or speech. Temporal state uses its own monotonic revision and merge policy so a stale snapshot cannot roll activity markers or eligible conditions backward.

Bounded temporal meaning is owned by the pure evidence and eligibility modules `packages/nan0-runtime/src/temporal/Nan0TemporalEngine.ts` and `packages/nan0-runtime/src/temporal/Nan0TemporalEventGenerator.ts`. The first owns objective phase, absence interval, clock/shutdown evidence, and sleep compatibility; the second stores only lived consequences such as waits, explicit Kyo return promises, multi-day rhythms, idle depth, and milestones. Both use stable evidence keys and persisted crossed markers, so a pass after downtime recovers every missed threshold exactly once instead of relying on narrow polling windows. Neither generates dialogue, thoughts, decisions, or provider work. Eligible events become Nan0-owned `internal:temporal` observations and reuse the existing private Thought Engine, DecisionEngine, owner bridge, AIRI provider/stream/TTS path, and terminal persistence. Suspicious wall-clock jumps gate time-derived eligibility; timeline sequence remains authoritative. Pending Intentions retain exclusive intention lifecycle ownership.

Nan0's autonomous metabolism is a set of persisted cognition inputs, not a second assistant pipeline. `packages/nan0-runtime/src/emotional/Nan0EmotionalDynamics.ts` owns elapsed-time decay and sourced emotional consequences; `attention/Nan0AttentionEngine.ts` owns JSON-safe streams, bounded internal-observation queues, cooperative focus, and terminal settlement; `prediction/Nan0PredictionEngine.ts` consumes only attended observations and persists serializable expectations/patterns/beliefs; `goals/Nan0GoalEngine.ts` extends the canonical `Nan0Goal` schema through metadata and requires repeated emotional evidence before commitment; and `heartbeat/Nan0HeartbeatEngine.ts` is only an idempotent, jittered, serial scheduler. `Nan0Kernel.evaluateMetabolism()` is the sole composition boundary: decay/evidence/queue selection happen before one selected observation enters `prepareTurn()`, and speech remains impossible without a persisted thought and allowed DecisionEngine result. The heartbeat never imports providers, session stores, rendering, TTS, or output APIs. The root renderer merely requests owner evaluation and hands a sanitized allowed expression back through the existing `triggerOnly` AIRI lifecycle; real user inscriptions notify presence timers through `packages/stage-ui/src/stores/nan0-input-presence.ts`.

Nan0's local observability boundary is `packages/nan0-runtime/src/diagnostics/Nan0KernelObservatory.ts`. The runtime creates privacy-gated, correlated diagnostic events and a bounded asynchronous sink without importing Node APIs; `packages/stage-shared/src/nan0-diagnostics.ts` is the typed Eventa contract, and Electron main owns PowerShell formatting plus JSONL filesystem writes in `apps/stage-tamagotchi/src/main/services/airi/nan0-diagnostics.ts`. The heartbeat scheduler is the sole terminal-diagnostic owner and completes every started tick exactly once as `SPEAK`, `CHOSEN_SILENCE`, `NO_COGNITION`, `SUPPRESSED_SILENCE`, `WAIT`, `ACT`, `BODY_EXPRESSION`, `STALE`, or `FAILURE_SILENCE`. Diagnostics are local, disabled by default, read-only, and unable to alter thought, decision, expression, persistence, or output behavior; full private text requires explicit opt-in.
...
Nan0's durable future cognitive commitments are owned by `packages/nan0-runtime/src/intentions/Nan0PendingIntentions.ts`. Pending Intentions remain distinct from Goals and executable action intents: they retain honest origin, typed restart-safe trigger evidence, cooldowns, bounded attempts, and terminal merge semantics. Only the authoritative `#/chat` owner evaluates eligibility and creates Nan0-owned `internal:*` observations; the root-renderer heartbeat consults persisted `nextEvaluationAt` and makes no provider call itself. Due observations reuse the normal private Thought Engine and authoritative DecisionEngine. A proven SPEAK result crosses the owner bridge only as sanitized provenance and enters AIRI through `ingest('', { triggerOnly: true })`, preserving the existing provider, stream, TTS, and completion path without fabricating or persisting a Kyo message. SILENCE and WAIT remain non-speaking, and ACT may persist bounded authority but does not execute autonomously in this phase.
