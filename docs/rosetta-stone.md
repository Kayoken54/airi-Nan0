...
Nan0's private thought boundary is owned by `packages/nan0-runtime/src/thought/Nan0ThoughtEngine.ts`. `Nan0Kernel.prepareTurn()` first persists the canonical observation and prepared turn, then uses AIRI's configured provider/model for a hidden thought inference and merge-persists exactly one versioned `Nan0Thought` before returning. The `#/chat` renderer is the sole Nan0 kernel and persistence owner; the root AIRI chat renderer registers the lifecycle hooks and communicates with that owner through the narrow request/acknowledgement bridge in `packages/stage-ui/src/stores/nan0-bridge.ts`. Only a bounded prepared-turn proxy crosses that bridge, never raw `privateText`. A `SPEAK` thought contributes its sanitized interpretation directive to AIRI's existing visible stream; `SILENCE`, `ACT`, and `WAIT` set a pre-send response disposition that uses the existing assistant-silence lifecycle without starting the visible provider stream, and ACT/WAIT do not execute tools. Completion is acknowledged only after the owner persists it. A failed thought terminates the turn without fabricated speech. Normal spoken turns therefore use two provider calls: private thought followed by AIRI's existing streamed outward-expression call.
